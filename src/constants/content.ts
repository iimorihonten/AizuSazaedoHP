import { getImagePath } from "./config";

export const content = {
  ja: {
    title: "会津さざえ堂",
    subtitle: "国指定重要文化財",
    description: "世界唯一\n二重螺旋構造の木造建築",
    image: getImagePath("/lovable-uploads/a338b15d-845e-46e5-b409-6a7e79b38d2a.png"),
    menu: {
      news: "お知らせ",
      about: "会津さざえ堂とは",
      "sazaedo-iimori": "さざえ堂と飯盛家",
      visiting: "ご拝観について",
      honten: "山主飯盛本店",
      contact: "お問い合わせ"
    },
    quickAccess: {
      title: "基本情報",
      hours: {
        title: "拝観時間",
        content: "8:15～日没（4月～11月）\n9:00～16:00（12月～3月）",
      },
      fees: {
        title: "拝観料",
        content: "大人：400円\n大学生・高校生：300円\n小中学生：200円",
      },
      access: {
        title: "アクセス",
        content: "バス\nハイカラさん・あかべぇ「飯盛山下」下車、徒歩7分\n自動車\n磐越自動車道　会津若松ICより約15分\n弊社での駐車場のご用意はございません。\n飯盛山入口周辺のお店の駐車場、もしくは\n市営の駐車場をご利用の上お越しください。",
      },
    },
  },
  en: {
    title: "Aizu Sazaedo",
    subtitle: "Important Cultural Property of Japan",
    description: "World's Only\nDouble-helix Wooden Architecture",
    image: getImagePath("/lovable-uploads/a338b15d-845e-46e5-b409-6a7e79b38d2a.png"),
    menu: {
      news: "News",
      about: "About Aizu Sazaedo",
      "sazaedo-iimori": "Sazaedo and Iimori Family",
      visiting: "Visiting Information",
      honten: "Iimori Honten Shop",
      contact: "Contact"
    },
    quickAccess: {
      title: "Basic Information",
      hours: {
        title: "Opening Hours",
        content: "8:15-Sunset (April-November)\n9:00-16:00 (December-March)",
      },
      fees: {
        title: "Admission",
        content: "Adults: ¥400\nUniversity/High School Students: ¥300\nElementary/Junior High School Students: ¥200",
      },
      access: {
        title: "Access",
        content: "By Bus:\nTake 'Haikara-san/Akabee' bus to 'Iimoriyama-shita' stop, 7-minute walk\nBy Car:\nAbout 15 minutes from Aizu Wakamatsu IC on Ban-etsu Expressway\nWe do not have our own parking lot.\nPlease use the parking lots of shops near the entrance of Mt. Iimoriyama\nor the municipal parking lot.",
      },
    },
  },
};
